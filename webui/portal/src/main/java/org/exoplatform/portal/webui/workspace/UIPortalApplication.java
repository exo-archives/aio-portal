 ...
 * tree).
 * 
 * The UIPortalApplication is responsible to build its subtree according to some
 * configuration parameters. If all components are displayed it is composed of 3
 * UI components:
 *  - UIControlWorkSpace : the left expandable column that can contains widgets
 * containers and the start menu - UIWorkingWorkSpace: the right part that can
 * display the normal or webos portal layouts - UIPopupWindow: a popup window
 * that display or not
 * 
 */
@ComponentConfigs( {
    @ComponentConfig(lifecycle = UIPortalApplicationLifecycle.class, template = "system:/groovy/portal/webui/workspace/UIPortalApplication.gtmpl", initParams = @ParamConfig(name = "public.showControlWorkspace", value = "true")),
    @ComponentConfig(id = "office", lifecycle = UIPortalApplicationLifecycle.class, template = "system:/groovy/portal/webui/workspace/UIPortalApplication.gtmpl", initParams = @ParamConfig(name = "public.showControlWorkspace", value = "false")) })
public class UIPortalApplication extends UIApplication {
   ...
   * that will be aggregated in the portal page.
   * 
   ...
   * request is extracted from the PortalRequestContext 3) Then according to the
   * context path, either a public or private portal is initiated. Usually a
   * public portal does not contain the left column and only the private one has
   * it. 4) The skin to use is setup 5) Finally, the current component is
  ...
  public UIPortalApplication(InitParams initParams) throws Exception {
    ...
    userPortalConfig_ = (UserPortalConfig) context
        .getAttribute(UserPortalConfig.class);
    ...
    LocaleConfig localeConfig = localeConfigService
        .getLocaleConfig(userPortalConfig_.getPortalConfig().getLocale());
   ...
   */
  @SuppressWarnings("hiding")
  private void initPublicPortal(PortalRequestContext context,
      InitParams initParams) throws Exception {
    if ("true".equals(initParams.getParam("public.showControlWorkspace")
        .getValue())) {
      addChild(UIControlWorkspace.class, UIPortalApplication.UI_CONTROL_WS_ID,
          null);
    }
  ...
  /**
   * A private portal always get the left column and the main area in the
   * center.
   * 
   * @param context
   * @throws Exception
   */
  @SuppressWarnings("hiding")
  private void initPrivatePortal(PortalRequestContext context, String remoteUser)
      throws Exception {
   ...
   * 
   * 1) A UIPortal child which is filled with portal data using the
   * PortalDataMapper helper tool 2) A UIPortalToolPanel which is not rendered
   * by default
   * 
  ...
  private void addWorkingWorkspace(PortalRequestContext context)
      throws Exception {
    UIWorkspace uiWorkingWorkspace = createUIComponent(UIWorkspace.class,
        UIPortalApplication.UI_WORKING_WS_ID, null);
    ...
    uiWorkingWorkspace.addChild(UIPortalToolPanel.class, null, null)
        .setRendered(false);
  ...
  public void processDecode(WebuiRequestContext context) throws Exception {
    ...
    PageNodeEvent<UIPortal> pnevent = new PageNodeEvent<UIPortal>(uiPortal,
        PageNodeEvent.CHANGE_PAGE_NODE, null, nodePath_);
   ...
   * for a full page render or in the case of an AJAX call
   * 
   * The first request, Ajax is not enabled (means no ajaxRequest parameter in
   * the request) and hence the super.processRender() method is called. This
   * will hence call the processrender() of the Lifecycle object as this method
   * is not overidden in UIPortalApplicationLifecycle. There we simply render
   * the bounded template (groovy usually). Note that bounded template are also
   ...
   * the URL. In that case the algorithm is a bit more complex:
   * 
  ...
  private String getAddSkinScript(List<UIComponent> updateComponents) {
    ...
    for (SkinConfig ele : skins) {
      b.append("eXo.core.Skin.addSkin('").append(ele.getId()).append("','")
          .append(ele.getCSSPath()).append("');\n");
    }
    return b.toString();
  }
